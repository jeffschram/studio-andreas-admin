    ...
    <div style={{ background: C.cream, minHeight: "100vh", fontFamily: "system-ui, -apple-system, sans-serif", color: C.black }}>
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "48px 32px 80px" }}>
        ...
        <div style={{ marginBottom: 48 }}>
          ...
          <h1 style={{ fontSize: 36, fontWeight: 800, color: C.black, margin: 0, lineHeight: 1.15 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 24 }}>
            {[
              ...
              <div key={c.label} style={{ background: C.green, borderRadius: 8, padding: "20px 16px", borderTop: `3px solid ${C.orange}` }}>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", color: C.orange, textTransform: "uppercase", marginBottom: 8 }}>
                  {c.label}
                </div>
                <div style={{ fontSize: 13, color: "#d4e0d4", lineHeight: 1.55 }}>{c.desc}</div>
              </div>
            ))}
          </div>
        ...
        <Section number="02" title="The Full Workflow">
          <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 28, color: "#555" }}>
                ...
                <div style={{ position: "relative" }}>
                  {step.auto && (
                    <div style={{
                      ...
                      background: C.orange, color: C.black, fontSize: 9, fontWeight: 800,
                      letterSpacing: "0.08em", padding: "2px 6px", borderRadius: 3, whiteSpace: "nowrap"
                    }}>AUTO</div>
                  )}
                  <div style={{
                    background: C.green, borderRadius: 8, padding: "12px 14px", textAlign: "center",
                    minWidth: 90, border: step.auto ? `2px solid ${C.orange}` : `2px solid #4a6349`
                  }}>
                    <div style={{ width: 22, height: 22, borderRadius: "50%", background: C.orange, color: C.black, fontSize: 11, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 6px" }}>{step.n}</div>
                    <div style={{ fontSize: 11, color: C.white, lineHeight: 1.4, whiteSpace: "pre-line" }}>{step.label}</div>
                  </div>
                </div>
                {i < arr.length - 1 && <div style={{ color: C.orange, fontSize: 18, fontWeight: 300, marginTop: 8 }}>→</div>}
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <div style={{ background: C.white, border: "1px solid #ddd0b8", borderRadius: 8, overflow: "hidden" }}>
              <div style={{ background: C.orange, padding: "10px 16px", fontSize: 12, fontWeight: 700, color: C.black }}>What Happens Automatically</div>
              <ul style={{ margin: 0, padding: "16px 16px 16px 32px", fontSize: 13, lineHeight: 2, color: C.black }}>
            ...
            {[
              ...
              <div key={step.n} style={{ display: "flex", gap: 16 }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: C.orange, color: C.black, fontSize: 13, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{step.n}</div>
                  {i < arr.length - 1 && <div style={{ width: 2, flex: 1, background: "#ddd0b8", margin: "4px 0" }} />}
                </div>
                <div style={{ paddingBottom: 20 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.black, marginBottom: 3 }}>{step.title}</div>
        ...
        <Section number="04" title="The Google Sheet">
          <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 24, color: "#555" }}>
            Three tabs — each plays a distinct role in the payroll system.
          </p>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 24 }}>
            {[
              { tab: "Pay Periods", items: ["Pre-loaded payroll schedule", "Defines all 2-week periods", "Read-only — do not edit", "Claude reads this to detect the current period"] },
              { tab: "Instructors", items: ["Source of truth for all staff", "Edit: Include in Payroll (Yes / No)", "Edit: Tech Hours Rate ($/hr)", "To deactivate: change Yes → No"] },
              { tab: "Payroll", items: ["Auto-fills after submissions", "TRUE = session confirmed", "DISPUTED = instructor flagged", "Tech Hours appear below each instructor's Acuity sessions"] },
            ].map((t) => (
              <div key={t.tab} style={{ background: C.white, border: "1px solid #ddd0b8", borderRadius: 8, overflow: "hidden" }}>
                <div style={{ background: C.green, borderBottom: `3px solid ${C.orange}`, padding: "12px 16px", fontSize: 13, fontWeight: 700, color: C.white }}>{t.tab}</div>
                <ul style={{ margin: 0, padding: "12px 16px 12px 28px", fontSize: 12.5, lineHeight: 1.9, color: "#333" }}>
        ...
        <Section number="05" title="The Instructor Experience">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              <div key={card.title} style={{ background: C.white, border: "1px solid #ddd0b8", borderLeft: `3px solid ${C.orange}`, borderRadius: "0 8px 8px 0", padding: "14px 16px" }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: C.black, marginBottom: 4 }}>{card.title}</div>
        ...
        <Section number="06" title="Managing Instructors & Pay Rates">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[
              ...
              <div key={item.title} style={{ background: C.white, border: "1px solid #ddd0b8", borderRadius: 8, overflow: "hidden" }}>
                <div style={{ background: C.green, padding: "9px 16px", fontSize: 12, fontWeight: 700, color: C.orange, letterSpacing: "0.03em" }}>{item.title}</div>
                <div style={{ padding: "12px 16px", fontSize: 13.5, color: "#333", lineHeight: 1.7 }}>{item.desc}</div>
              </div>
            ))}
          </div>
        </Section>
...
function Section({ number, title, children }: { number: string; title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 56 }}>
      <div style={{ display: "flex", alignItems: "center", marginBottom: 24, background: C.green, borderRadius: 8, overflow: "hidden" }}>
        <div style={{ background: C.orange, padding: "12px 20px", fontSize: 26, fontWeight: 900, color: C.black, lineHeight: 1 }}>{number}</div>
        <div style={{ padding: "12px 20px", fontSize: 18, fontWeight: 700, color: C.white }}>{title}</div>
      </div>
      {children}
    </div>
  );
...
function Callout({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ background: C.offwhite, border: "1px solid #ddd0b8", borderRadius: 8, padding: "14px 18px", fontSize: 13.5, lineHeight: 1.7, color: C.black }}>
      {children}
    </div>
  );
}
